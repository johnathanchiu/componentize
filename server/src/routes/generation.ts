...
export function registerGenerationRoutes(server: FastifyInstance) {
  ...
  // Legacy: Stream generation in same request (for backwards compatibility)
  // This can be deprecated once frontend uses the new pattern
  server.post<{ Params: { id: string }; Body: { prompt: string } }>(
    '/api/projects/:id/generate-legacy',
    async (request, reply) => {
      const { id } = request.params;
      const { prompt } = request.body;
