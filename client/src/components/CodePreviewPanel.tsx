...
export function CodePreviewPanel() {
  ...
  // Only show when editing or fixing
  const isVisible = (generationMode === 'edit' || generationMode === 'fix') && editingComponentName;
    ...
    <div className="w-80 border-l border-neutral-200 bg-white flex flex-col">
      ...
      <div className="px-4 py-2 border-t border-neutral-100 bg-neutral-50">
        <p className="text-xs text-neutral-400">
          {generationMode === 'fix' ? 'Fixing errors...' : 'Describe changes in the prompt below'}
        </p>
      </div>
    </div>
  );
